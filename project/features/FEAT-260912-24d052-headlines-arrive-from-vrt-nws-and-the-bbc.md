...
- [ ] `news_search` narrows by source slug, by a `since` date, and by a case-insensitive `query` over title and summary
- [ ] `news_search` defaults to `detail="concise"` — summary cut to 200 characters, no link — and `detail="full"` carries both whole
- [ ] `candidates()` stays a plain list; only `news_search` carries `unavailable`
...
- [ ] A feed is waited on for 10 seconds and asked once — nothing retries
- [ ] The Slack line carries the source name and a sentence, never the feed URL or the response body
...
- [ ] `docs/sources.md` describes the news source, its two settings, and both of its failure paths
