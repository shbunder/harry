...
- [x] On the real stack, a connector's own `.env.local` is read through a read-only mount of `.harry/`, and an environment variable still wins over it
...
- [x] On the NUC, starting with no saved session, a page built with a De Tijd story, deliver=false, carries its whole article, and the session exists only on the data volume
...
- **2026-09-17** — Reflection: merged as f25de97 after three verification passes (plan-verifier WARN; code review with 1 Critical — a URL-encoded email in fixtures, removed and the branch history rewritten before the merge; pre-close REQUEST CHANGES twice, each acted on, with about 100 mutations across the feature each red). Traceability: every criterion on stories f048f5, daf947, 133f51 and c04a3c maps to a named test or a by-inspection reason; ebeb3f waits on mode 600 on the NUC and 969f41 on the deploy. Degraded paths tested: 16, through news_article over MCP or load(). Scope drift: init on both stacks, the ERR_ABORTED retry and the 10-minute rest after a slow failure came from review findings; no non-goal was built. Owed: Chromium runs as root without its sandbox, accepted by the owner and scheduled as FEAT-260917-250f5a.
- **2026-09-17** — DE TIJD IS LIVE ON THE NUC, 2026-09-17. Deployed harry:6220e5c from the main checkout. /health: 16 loaded, 0 skipped — icloud, remarkable, slack and tijd all came up from their own .env.local through the read-only mount, and the load order shows tijd before news. news_article for tijd-2026-09-17-ai-nadert-het-punt-waarop answered available: true with 11,760 characters of prose, from Harry's own login: /data/tijd held nothing before, and now holds storage-state.json (mode 600) and logged-in-at, both written at 20:47. A page built through scripts/call_tool.py with that story and deliver=false came out at 8 pages in 20 seconds, /data/digest/2026-09-17.pdf, nothing pushed. Nothing reached Slack. ONE BUG FOUND BY DEPLOYING, fixed first and merged as 6220e5c: every connector's .env.local on this machine is written 'export KEY=value', and the reader of a capability's pair did not strip 'export' the way both readers of the root pair do — so icloud, remarkable, slack, digest_build and De Tijd's feed all read as unconfigured with their credentials right there. Reproduced, fixed, guarded by tests/test_capability_config.py::test_an_exported_key_in_this_machine_s_file_is_read_like_a_plain_one. Also on the day: docker compose refused the root .env.local over a stray line reading 'export' alone, which the owner removed.
