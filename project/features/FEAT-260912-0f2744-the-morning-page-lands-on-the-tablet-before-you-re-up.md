...
- **2026-09-15** — Found while building a page by hand on the NUC, and not fixed here because this feature is closed. digest_build's TOOL.md body offers Claude the topic 'sport', and marks.py renders that topic as the heading 'Basketball' — a deliberate reader preference, with page.py's docstring discussing 'a day with no basketball, which is most days'. But the body Claude chooses from never says so, so a football story filed under 'sport' arrives beneath a Basketball heading. Seen live: a Raheem Sterling piece did exactly that. One line in the TOOL.md body would close it — say what 'sport' means to this reader — and TOOL.md bodies are governed by .claude/rules/tool-design.md, so it wants its own small change rather than a drive-by edit.
- **2026-09-16** — The trigger path, settled for one of its two routes (2026-09-16). A headless 'claude -p' on the NUC called digest_list_candidates on the containerised Harry over localhost with bearer auth and got 'PLACE=Leuven HEADLINES=40 AGENDA=false' back, success, in 8.6s. Configured: an MCP config with 'Authorization: Bearer ${HARRY_API_TOKEN}', '--allowedTools mcp__harry__digest_list_candidates', the existing subscription login on the NUC. Full account on FEAT-260913-fdd33f. The cloud-routine-through-the-tunnel route remains unproven. Which one the 06:30 page uses is an open decision with a real trade in it: the local route needs no public endpoint for a service holding a tablet token with no scopes, but puts the morning page on a Claude login on the NUC that expires quietly — which the deadline watchdog already covers, since a lapsed login produces exactly 'no page built by 07:00'.
- **2026-09-16** — Decided, later the same day: the 06:30 page is fired by a Claude CLOUD ROUTINE reaching Harry through its own Cloudflare tunnel, chosen by the operator — ADR-260916-7d4bd0. This settles the 'open decision with a real trade in it' in the note above. The local claude -p route stays proven and written down as the fallback: if a cloud routine cannot present the bearer token to Harry, the trigger moves there and the tunnel is deleted. Building the route is FEAT-260913-fdd33f; this feature's routine brief and schedule build on it.
