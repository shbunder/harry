...
- [ ] `.mcp.json` carries a second server, `harry-remote`, at `https://harry.marcel-bot.com/mcp` with header `Bearer ${HARRY_API_TOKEN}` and no fallback value; the `harry` entry is unchanged
- [ ] A cloud routine run with no `HARRY_API_TOKEN` in its environment is refused, and the probe's call log shows no successful call from it
...
- [ ] If the routine cannot reach the probe, the reason is recorded — the token not reaching the header, the plan, or the network — and the `harry` tunnel and its DNS record are deleted
