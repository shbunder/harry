...
- [x] `_retire` lists the folder with `refresh=True`
- [x] The test stand-in can lag: without `refresh` it answers from a snapshot, with it from the tablet — and every existing test still passes against it
- [x] A test pushes a name whose older copy the stand-in's snapshot does not know about, and fails when `_retire` does not refresh
- [x] The runbook and `docs/sources.md` say what a duplicate means now
