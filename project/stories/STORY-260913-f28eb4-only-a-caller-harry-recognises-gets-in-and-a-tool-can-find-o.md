...
- [ ] A request with a token that is not the configured one is refused
- [ ] A request with the configured token gets the roster
- [ ] An unset HARRY_API_TOKEN authorises nobody, including the owner
- [ ] A tool whose signature declares a principal is handed the caller, not a default
- [ ] principal does not appear in that tool's input schema — Harry fills it, never the caller
