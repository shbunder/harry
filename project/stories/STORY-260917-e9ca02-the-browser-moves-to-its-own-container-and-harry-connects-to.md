...
- [x] The image installs its browsers where any user can read them, and the browser container runs as a non-root user with every capability dropped
- [x] `docker-compose.yml` gains a browser service from the same image: no `.harry/` mount, no root `.env.local`, no data volume, no published port, and a test in the gate fails if it gains any of them
