...
- **2026-09-15** — Feature criterion 8 is no longer ticked on prose. The in-container page build is a live test that execs scripts/call_tool.py in the running container, reaches the real feeds, and asserts the page lands under /data with pushed=false — and it needs no credential, which is what makes it the right test for this story's remaining state. It skips with a sentence when the stack is down rather than starting it. Ran for real: the page is /data/digest/2026-09-16.pdf.
- **2026-09-16** — Status is In Progress rather than Backlog: four of five criteria are ticked and two commits have shipped for it. The remaining one needs a real credential placed on the NUC, which is the operator's to do.
