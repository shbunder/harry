...
- **2026-09-15** — The docker-compose project name was unpinned, which is a sharper version of the worktree trap already noted here. Compose names the project after the directory it runs in, and that name prefixes every volume — so 'make up' from a worktree used 2a6ce1_harry-data and from /home/shbunder/projects/harry it would use harry_harry-data. Two stores, both healthy, nothing saying which one yesterday's page and the watchdog's memory are in. Fixed with a top-level 'name: harry' and verified from the worktree: the volume is harry_harry-data either way.
