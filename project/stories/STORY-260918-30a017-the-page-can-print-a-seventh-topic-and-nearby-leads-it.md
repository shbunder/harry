...
- [x] `marks.py` carries `regional` first, with its own shape, and a test fails if it is dropped or moved behind `belgium`
...
- [x] Front-page picks sort with every `regional` ahead of every `belgium`, and a test fails if the order changes
- [x] The second sheet heads its first section "Nearby" when anything regional is in it
...
- [x] `.harry/jobs/morning-page/JOB.md` describes the topic, names Oostende, Leuven and Holsbeek, and says to leave the section out on a day with none
