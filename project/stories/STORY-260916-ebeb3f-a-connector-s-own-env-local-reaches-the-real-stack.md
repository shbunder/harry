...
- [ ] When it is set, a capability's `.env.local` at `<dir>/connectors/<name>/.env.local` — the kind's folder name, plural, as under `.harry/` — is read, and wins over the `.env.local` beside the capability
- [ ] A connector and a tool with the same name read different mounted files
- [ ] The environment variable still wins over the mounted file, and a principal's own file still wins over both
- [ ] When it is empty, nothing changes: a file at the would-be mounted path is not read
- [ ] Nothing but `.env.local` is read from the mounted directory — a `.env` placed there is ignored
- [ ] Through `load()`, a connector whose required settings are only in the mounted directory loads, and is skipped when the setting is empty
- [ ] `docker-compose.yml` mounts `./.harry` read-only on the `harry` service at the path its `HARRY_CAPABILITY_SETTINGS_DIR` names; `harry-dev` mounts nothing but its volume and sets nothing
- [ ] `tests/test_deployment.py::test_each_stack_keeps_its_data_on_a_named_volume_of_its_own` is narrowed, not deleted: data still lives on a named volume of each stack's own, and a new test proves the only other mount is `./.harry`, read-only, on the real stack
- [ ] In the built image, a container with a `.harry`-shaped directory mounted read-only and the setting pointing at it loads a connector from it (live)
- [ ] `docs/operating.md` says where a credential goes on the NUC, in the present tense; that each connector's `.env.local` should be mode 600; and that the dev stack still reads `.env.dev.local`
