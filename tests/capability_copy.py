...
test, it makes the suite answer differently on every laptop: on the machine this was written,
`.harry/connectors/news/.env.local` names a third feed, and thirty-one news tests went from
green to reaching `tijd.be` and failing. On a machine with no such file they all pass and
nothing says why.
...
what the capability under test is configured to do. `__pycache__` is the same class of leak —
a stale `.pyc` compiled against code that has since changed.
"""
