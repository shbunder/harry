...
NEARBY_FEEDS = f'FEEDS=vrt=VRT NWS={VRT}|rob=ROB tv={ROB}|kw=KW West-Vlaanderen={KW}\n'
"""The national feed and the two that carry the towns it misses: ROB tv for the Leuven area,
including Holsbeek, and KW for the coast. Both RSS 2.0; VRT is Atom — measured 2026-09-18."""
...
def test_the_regional_feeds_are_read_beside_the_national_one(news):
    """VRT's own regional feeds answer 410 Gone, so the towns come from ROB tv for the Leuven
    area and KW for the coast. Both are RSS 2.0, the format the BBC feed already proves."""
    ...
    assert set(first) == {'VRT NWS', 'ROB tv', 'KW West-Vlaanderen'}, first
    assert first['ROB tv'].startswith('rob-')
    assert first['KW West-Vlaanderen'].startswith('kw-')
...
def test_a_regional_feed_that_is_down_costs_only_itself(news):
    ...
    serving(**{VRT: 'vrt-nws.xml', KW: 'kw-west-vlaanderen.xml'})
