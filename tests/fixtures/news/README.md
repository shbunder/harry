...
| `hln-leuven.xml` | `www.hln.be/leuven/rss.xml`, recorded 18 September 2026 — RSS 2.0 | 30 items cut to 8 |
| `hln-oostende.xml` | `www.hln.be/oostende/rss.xml`, recorded 18 September 2026 — RSS 2.0 | 30 items cut to 8 |
...
**The three regional feeds were recorded because VRT's own are gone.** `nl.rss.articles_regio_*`
answers **410 Gone**, and Focus-WTV and Nieuwsblad both answer **403** to a scripted client —
the De Tijd problem. These three answered 200. HLN is paywalled, so its articles read back as
the feed's summary rather than full text; the feeds themselves are public.
