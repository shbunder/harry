...
| Spike | Story | Settles |
|---|---|---|
| `test-server` | `835929` | A FastMCP server with a `ping` tool and a five-minute `sleep` tool, on localhost and through the tunnel. The next two both call it. |
...
| `blocking-call` | `f7cda6` | Whether a tool call held open for five minutes returns. **The whole Slack loop rests on it**; a failure makes `ask_human` a request id plus a poll. |
